...
description: Archive DeepSeek Harness Agent Notes. Use when adding, auditing, pruning, archiving, restoring, or reviewing Agent Notes in a deepseek-harness checkout; checks every new note for superseded active records, classifies implemented notes by future decision value, deletes rejected notes that no longer prevent a tempting fallacy, and applies the frozen archived/{kind} triplet and manifest rules.
---
...
1. Move the complete `foo.md`, `foo.zh.md`, and `foo.i18n.yaml` triplet from `.agents/notes/implemented/<kind>/` to `.agents/notes/archived/<kind>/`; `implemented` is deliberately absent from the archive path.
...
Run the archive verifier's focused test, `pnpm run verify-archived-agent-notes`, `pnpm run doc-sync`, `pnpm run lint`, and `git diff --check`; select any additional evidence through the repo-local `dsh-pre-push-checks` skill when working inside a deepseek-harness checkout.
...
Outside a deepseek-harness workspace the same archive rules run from this skill:

```bash
node scripts/verify-archived-agent-notes.mjs --root .          # check seals and triplets
node scripts/verify-archived-agent-notes.mjs --root . --write  # append seals for new triplets
```

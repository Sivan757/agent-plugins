...
import { CONFIG_UI as MYSQL_CONFIG_UI } from './config-ui.js';
...
const REASON_NEEDS_CONFIG = MYSQL_CONFIG_UI.reason ?? '';
...
program
  .name('mysql')
  .description('MySQL query executor')
  .version('0.12.1')
...
program
  ...
  .action(async (options: { ui?: boolean }) => {
    let config = await readStoredConfig<MySQLConfig>('mysql');
    ...
    if (options.ui) {
      ...
      config = await readStoredConfig<MySQLConfig>('mysql');
    }
    ...
    for (const line of summarizeConfig(config, { spec: MYSQL_CONFIG_UI.spec })) console.log(line);

 ...
 *   plugins/config-center/ui/dist/index.html (dev).
...
export interface ConfigUIOptions {
  ...
  setupCommand?: string;
  /**
   * Why this plugin needs the form, in the user's terms. Printed whenever the
   * form is opened automatically, so the reader knows what is missing rather
 ...
 *    - Dev (tsx): thisDir is plugins/config-center/src/, so ../ui/dist/ resolves to
 *      plugins/config-center/ui/dist/index.html.
...
export function launchUI(
  ...
  // Serving the form and opening a browser are separable: a headless host or a
  // test wants the URL without a window. The URL always goes to stderr, so the
  // operator can open it by hand either way.
  const open = options?.open ?? !process.env.AGENT_PLUGINS_NO_BROWSER;
  // Same reasoning as `open`: a test or a CI run needs the form to end on a
  // schedule it controls rather than the human-sized default.
  const timeoutMs =
    options?.timeoutMs ??
    (Number(process.env.AGENT_PLUGINS_UI_TIMEOUT_MS) || 5 * 60 * 1000);
  ...
  // A programmatic launch (no browser) is a test or a preview, and it will
  // render whatever credentials are on disk. Say so, loudly: this is how a
  // preview script once put a real secret into a transcript. The operator's
  // path is deliberately not printed.
  const hasStoredConfig = Object.keys(existing).length > 0;
  if (!open && hasStoredConfig && !process.env.AGENT_PLUGINS_CACHE_DIR) {
    output.stderr(
      `[config-center] WARNING: rendering the stored configuration for "${pluginName ?? 'unknown'}". ` +
        `For previews and tests set AGENT_PLUGINS_CACHE_DIR to a scratch directory — changing HOME ` +
        `inside a script does not isolate it.\n`,
    );
  }

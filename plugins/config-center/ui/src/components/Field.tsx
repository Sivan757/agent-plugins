...
export function Field({ props }: BaseComponentProps<FieldProps>) {
  ...
  // Conditional fields: keep the form focused on the mode the user picked.
  const gate = props.visibleWhen;
  if (gate) {
    const raw = get(gate.statePath);
    const value = raw === null || raw === undefined ? '' : String(raw);
    const allowed = gate.oneOf ?? (gate.equals != null ? [gate.equals] : null);

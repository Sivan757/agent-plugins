...
Apifox CLI skills, ported from the official [apifox/apifox-cli-skills](https://github.com/apifox/apifox-cli-skills) into an Agent Plugins skill-only plugin. Upstream has no LICENSE file; these are official Apifox-authored skill documents (Chinese), carried over with the in-plugin cross-references retargeted and the CLI checkup and API-lifecycle workflow folded into `apifox-cli`.
...
| Skill | Purpose |
| --- | --- |
| `apifox-cli` | Entry point and golden path: the end-to-end API lifecycle (requirements → endpoint/schema design → environment → mock → test cases → doc export/publish → branch merge), project resource management, login/project setup, the mandatory write-validation flow, AI-branch and write-permission rules, and the must-ask-user list. Its `references/checkup.md` appendix covers CLI usage checks and version confirmation — commands succeed but the page shows nothing, resources not found after create, missing reports, agentHints/help inconsistencies, or a stale local CLI. |
...
The default verb is `apifox-cli` as the entry point — it carries the end-to-end API lifecycle, so a "create a set of APIs and fill in tests/mocks/docs" request runs as one chain instead of separate steps. Load the domain-specific skill for the business area (for example `apifox-test-scenario` for scenario modeling, `apifox-branch` for branch work). For any `create`/`update`, run the mandatory validate-then-write flow: `apifox cli-schema get <schemaKey>` → build the JSON payload file → `apifox cli-schema validate <schemaKey> --file <path>` → only then run the real command, and follow `agentHints.nextSteps` from the JSON output afterwards.

## Fallback primitives

`apifox-cli/references/checkup.md` is the diagnostic surface for when a command reports success but the result is not visible, readable, or reproducible.

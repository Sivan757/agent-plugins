...
Outside a deepseek-harness workspace these documentation checks run from this skill:

```bash
node scripts/verify-doc-budgets.mjs --root . [--list]  # word ceilings from scripts/doc-budgets.manifest.json
node scripts/doc-sync.mjs --root .                     # budgets + bilingual pairing + archived notes
```

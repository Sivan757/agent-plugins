...
// src/notebook.ts
var DEFAULT_CONTEXT_BUDGET = 12e3;
var DEFAULT_TOPIC_BYTES = 4096;
var DEFAULT_TOPIC_LIMIT = 5;
var INDEX_LINE_LIMIT = 200;
var INDEX_BYTE_LIMIT = 25e3;
var MAX_ENTRY_CHARS = 2e3;
var REQUIRED_SECTIONS = ["Why", "Where", "What", "Not"];
var MISTAKE_SECTIONS = [
  "Problem",
  "Root Cause",
  "Fix",
  "Verification"
...
var MEMORY_TYPES = ["user", "style", "rule", "project", "mistake", "reference"];
var CONFIDENCE_VALUES = ["high", "medium", "low"];
var SENSITIVITY_VALUES = ["public", "internal", "redacted"];
var CONTEXT_FORMATS = ["text", "hook", "json"];
var INSIGHT_PLATFORMS = ["claude", "codex", "auto"];
...
function resolveNotebookPaths(options = {}) {
...
function singleScopes(scope = "both") {
  if (scope === "both") return ["global", "project"];
  return [scope];
}
function rootFor(paths, scope) {
  return scope === "global" ? paths.global : paths.project;
}
async function initNotebook(options = {}) {
...
async function addMemory(options) {
  ...
  const now = localIsoString(/* @__PURE__ */ new Date());
...
function buildTopicBody(type, content, scope) {
...
function serializeTopic(frontmatter, body) {
  const lines = Object.entries(frontmatter).filter(([, value]) => value !== void 0 && value !== "").map(([key, value]) => `${key}: ${String(value).replace(/\n/g, " ")}`);
  return `---
${lines.join("\n")}
---
...
async function updateIndex(root, topicPath, fm) {
  await ensureFile(root.index, "# Notebook Index\n\n");
  const rel = relative(root.root, topicPath);
  const line = `- [${fm.title ?? basename(topicPath)}](${rel}) - ${fm.description ?? ""}; ${fm.type ?? "memory"}; ${fm.confidence ?? "medium"}; updated ${localIsoString(/* @__PURE__ */ new Date()).slice(0, 10)}.`;
...
async function removeIndexEntries(root, paths) {
...
async function appendLedger(root, event) {
  await ensureFile(root.ledger, "# Notebook Ledger\n\n");
  const rel = relative(root.root, event.path);
  const source = event.source ? ` | source=${sanitizeInline(event.source)}` : "";
  await appendFile(
    root.ledger,
    `- ${localIsoString(/* @__PURE__ */ new Date())} | ${event.action} | ${event.scope}/${event.type ?? "memory"} | ${sanitizeInline(event.title)} | ${rel}${source}
`,
    "utf8"
  );
}
async function searchNotebook(options) {
  const paths = resolveNotebookPaths(options);
...
async function searchRoot(root, scope, options) {
...
async function deleteMemories(options) {
  const paths = resolveNotebookPaths(options);
  const matches = options.file ? await matchExplicitFile(options.file, paths, options) : (await searchNotebook(options)).matches;
  if (!options.yes) return { matches, deleted: [] };
  const deleted = [];
  for (const match of matches) {
    await unlink(match.path).catch(() => void 0);
...
async function matchExplicitFile(file, paths, options) {
  const resolved = isAbsolute(file) ? file : resolve(file);
  if (!existsSync(resolved)) return [];
  const content = await readFile(resolved, "utf8");
  const parsed = parseTopic(content);
  const st = await stat(resolved);
  const scope = resolved.startsWith(paths.project.root) ? "project" : "global";
  return [{
    scope,
    path: resolved,
...
async function contextNotebook(options = {}) {
  const paths = resolveNotebookPaths(options);
  const scopes = singleScopes(options.scope ?? "both");
...
async function auditNotebook(options = {}) {
  const paths = resolveNotebookPaths(options);
  const issues = [];
  for (const scope of singleScopes(options.scope ?? "both")) {
    const root = rootFor(paths, scope);
    ...
    for (const entry of entries) {
      ...
      const content = await readFile(path, "utf8");
      const parsed = parseTopic(content);
      for (const key of ["title", "type", "description"]) {
...
function collectSecretIssues(scope, file, content, issues) {
...
async function compactNotebook(options = {}) {
...
async function insightsNotebook(options = {}) {
  const paths = resolveNotebookPaths(options);
  ...
  const report = [
    "# Notebook Insights",
    "",
    `Generated at: ${localIsoString(/* @__PURE__ */ new Date())}`,
    `Scope: ${scope}`,
    "",
    "## Memory Candidates",
  ...
  await writeFile(reportPath, report, { encoding: "utf8", mode: 384 });
  return { reportPath, memoryCandidates, bootstrapCandidates, staleCandidates, frictionCandidates };
}
async function resummarizeNotebook(options) {
  const paths = resolveNotebookPaths(options);
  const root = rootFor(paths, options.scope);
  await ensureRoot(root);
  const transcriptPaths = await readTranscriptPaths(root.sources);
...
async function readTranscriptPaths(sourcesPath) {
...
async function collectTranscriptSnippets(paths, query) {
  const userTexts = [];
  const matches = [];
...
function extractTranscriptText(line) {
  try {
...
function slugify(input) {
  const slug = input.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").replace(/-{2,}/g, "-");
  return slug || "memory";
}
...
function freshnessWarning(mtimeMs) {
...
function normalizeTrailingNewline(input) {
...
function parseOptionalMemoryType(value) {
  return value === void 0 ? void 0 : parseMemoryType(value);
}
function parseConfidence(value) {
  if (CONFIDENCE_VALUES.includes(value)) return value;
  throw new Error(`--confidence must be one of: ${CONFIDENCE_VALUES.join(", ")}`);
}
function parseOptionalConfidence(value) {
  return value === void 0 ? void 0 : parseConfidence(value);
}
function parseSensitivity(value) {
  if (SENSITIVITY_VALUES.includes(value)) return value;
  throw new Error(`--sensitivity must be one of: ${SENSITIVITY_VALUES.join(", ")}`);
}
function parseContextFormat(value) {
  if (CONTEXT_FORMATS.includes(value)) {
    return value;
  }
  throw new Error(`--format must be one of: ${CONTEXT_FORMATS.join(", ")}`);
}
function parseInsightPlatform(value) {
  if (INSIGHT_PLATFORMS.includes(value)) {
    return value;
  }
  throw new Error(`--platform must be one of: ${INSIGHT_PLATFORMS.join(", ")}`);
}
function commonPathOptions(opts) {
  return {
...
async function main(argv = process.argv) {
  ...
  program2.name("notebook").description("Local file-based multi-level memory for Codex and Claude Code").option("--global-root <path>", "Override global notebook root").option("--project-root <path>", "Override project root");
  program2.command("init").option("--scope <scope>", "global|project|both", "both").action(async (opts) => {
  ...
  program2.command("context").option("--scope <scope>", "global|project|both", "both").option("--query <query>").option("--bootstrap-only").option("--include-pending").option("--budget <chars>", "context character budget", (value) => Number(value), DEFAULT_CONTEXT_BUDGET).option("--format <format>", "text|hook|json", "text").action(async (opts) => {
    const rootOpts = program2.opts();
  ...
  program2.command("add").requiredOption("--scope <scope>", "global|project").requiredOption("--type <type>", "user|style|rule|project|mistake|reference").requiredOption("--title <title>").requiredOption("--content <content>").option("--confidence <confidence>", "high|medium|low", "medium").option("--sensitivity <sensitivity>", "public|internal|redacted", "internal").option("--source <source>").action(async (opts) => {
    const rootOpts = program2.opts();
  ...
  program2.command("search").requiredOption("--query <query>").option("--scope <scope>", "global|project|both", "both").option("--type <type>").option("--confidence <confidence>").option("--before <date>").action(async (opts) => {
    const rootOpts = program2.opts();
    const result = await searchNotebook({
      ...commonPathOptions(rootOpts),
      scope: parseScope(opts.scope),
  ...
  program2.command("delete").requiredOption("--query <query>").option("--scope <scope>", "global|project|both", "both").option("--type <type>").option("--file <path>").option("--yes").action(async (opts) => {
    const rootOpts = program2.opts();
  ...
  program2.command("promote").requiredOption("--scope <scope>", "global|project").requiredOption("--query <query>").option("--yes").action(async (opts) => {
    const rootOpts = program2.opts();
  ...
  program2.command("audit").option("--scope <scope>", "global|project|both", "both").action(async (opts) => {
    const rootOpts = program2.opts();
  ...
  program2.command("compact").option("--scope <scope>", "global|project|both", "both").option("--target <target>").option("--yes").action(async (opts) => {
    const rootOpts = program2.opts();
  ...
  program2.command("insights").option("--scope <scope>", "global|project|both", "project").option("--since <duration>", "duration such as 30d", "30d").option("--platform <platform>", "claude|codex|auto", "auto").action(async (opts) => {
    const rootOpts = program2.opts();
  ...
  program2.command("resummarize").requiredOption("--scope <scope>", "global|project").requiredOption("--query <query>").action(async (opts) => {
    const rootOpts = program2.opts();

...
/**
 * postgresql.ts - PostgreSQL query executor.
 *
 * Usage:
...
import { CONFIG_UI as PG_CONFIG_UI } from './config-ui.js';
...
async function loadConnectionConfig(
  ...
  if (!connConfig) {
    const reloaded = await reconfigure<PostgresConfig>(
      'postgresql',
      PG_CONFIG_UI,
...
program
  .name('postgresql')
  .description('PostgreSQL query executor')
  .version('0.6.1');
...
program
  ...
  .action(async (options: { ui?: boolean }) => {
    ...
    let config = await readStoredConfig<PostgresConfig>('postgresql');
    ...
    if (options.ui) {
      ...
      config = await readStoredConfig<PostgresConfig>('postgresql');
    }
    ...
    for (const line of summarizeConfig(config, { spec: PG_CONFIG_UI.spec })) console.log(line);
...
program
  ...
  .action(async () => {
    const existing = await readStoredConfig<PostgresConfig>('postgresql');

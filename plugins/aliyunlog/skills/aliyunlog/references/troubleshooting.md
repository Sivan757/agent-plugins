...
- **Endpoint unreachable** - Verify the `endpoint` in `~/.cache/agent-plugins/aliyunlog.json` matches your SLS region (e.g., `cn-hangzhou.log.aliyuncs.com`)
...
- Config is stored globally at `~/.cache/agent-plugins/aliyunlog.json` — outside project directories, no gitignore needed

...
import { CONFIG_UI as ALIYUNLOG_CONFIG_UI, REASON_NEEDS_CONFIG } from './config-ui.js';
...
export interface AliyunLogConfig extends Record<string, unknown> {
  credentials: {
    accessKeyId: string;
...
program
  ...
  .action(async (options: { ui?: boolean }) => {
    let config = await readStoredConfig<AliyunLogConfig>('aliyunlog');
    ...
    if (options.ui) {
      ...
      config = await readStoredConfig<AliyunLogConfig>('aliyunlog');
    }
    ...
    for (const line of summarizeConfig(config, { spec: ALIYUNLOG_CONFIG_UI.spec })) console.log(line);
...
program
  ...
  .option('--terminal', 'use the legacy terminal wizard instead of the browser form')
  .action(async (options: { terminal?: boolean }) => {
    if (options.terminal) {
      // Kept for a host with no browser at all. The browser form is the default
      // because this one reads the AccessKey secret through stdin, where it can
      // end up echoed into a terminal log.
      await cmdSetup();
      return;
    }
    const existing = await readStoredConfig<AliyunLogConfig>('aliyunlog');

...
function resolvePromptId(db, idOrPrefix) {
  const row = db.prepare("SELECT id FROM prompts WHERE id = ? OR id LIKE ?").get(idOrPrefix, `${idOrPrefix}%`);
  return row ? row.id : null;
}
function extractRecord(rec) {
  const text = String(rec.prompt_text ?? rec.title ?? rec.slug ?? "").trim();
  if (!text) return null;
  const sig = createHash("sha256").update(text).digest("hex").slice(0, 16);
  return { text, sig };
}
function importRecord(db, rec) {
  const extracted = extractRecord(rec);
  if (!extracted) return false;
  const { text, sig } = extracted;
  const existing = db.prepare("SELECT id FROM prompts WHERE id LIKE ?").get(`${sig}%`);
...
`
  ...
  const total = db.prepare("SELECT COUNT(*) as n FROM prompts").get();
  output.stdout(`
${total.n} total prompts
`);
  db.close();
}
function cmdPromptSearch(query, output) {
  const db = openDB();
  let rows = [];
  let usedFTS = false;
  try {
...
function cmdPromptAdd(opts, output) {
  const db = openDB();
...
function cmdImageRate(promptId, scoreStr, output) {
...
`);
  ...
  const avgRow = db.prepare("SELECT AVG(score) as avg FROM ratings WHERE prompt_id = ?").get(fullId);
...
function cmdSourceDedup(output) {
  const db = openDB();
...
function configureCmdRecursive(cmd, output) {
  cmd.configureOutput({
...
function buildProgram(output) {
  const program2 = new Command();
  program2.name("pf").description(
    "Prompt Forge CLI - image-generation prompt library backed by local SQLite."
  );
  program2.command("init").description("Create the database, apply schema, and seed from bundled data/*.jsonl.").action(() => cmdInit(output));
  const promptCmd = program2.command("prompt").description("Manage prompts.");
  promptCmd.command("list").description("List prompts ordered by rating.").option("--category <category>", "Filter by category").option("--rating <rating>", "Minimum rating", (v) => parseFloat(v)).option("--limit <limit>", "Maximum number of results", "50").action(
  ...
  const imageCmd = program2.command("image").description("Manage generated images.");
  imageCmd.command("link <promptId> <imagePath>").description("Link an image file to a prompt.").action(
    (promptId, imagePath) => cmdImageLink(promptId, imagePath, output)
  );
  imageCmd.command("rate <promptId> <score>").description("Rate a prompt (score 1-5) and update its average.").action((promptId, score) => cmdImageRate(promptId, score, output));
  const sourceCmd = program2.command("source").description("Bulk import and dedup.");
  sourceCmd.command("import <file>").description("Import prompts from a JSONL file (sha256 dedup).").action((file) => cmdSourceImport(file, output));
  sourceCmd.command("dedup").description("Remove duplicate prompts by exact prompt_text match.").action(() => cmdSourceDedup(output));
  program2.command("serve").description("Start a minimal HTTP server returning /api/stats JSON.").option("--port <port>", "Port number", "8765").action((opts) => cmdServe(opts, output));
  configureCmdRecursive(program2, output);
  return program2;
}
async function main(argv, output = defaultOutput) {
  ...
  try {
    ...
    output.stderr(`${String(err?.message ?? err)}
`);
    return 1;
  }
}

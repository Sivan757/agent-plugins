...
import { CONFIG_UI as TICKTICK_CONFIG_UI, REASON_NEEDS_CONFIG } from './config-ui.js';
...
Examples:
  ...
  program
    .command('config')
    .description('Show the configured account (secrets masked)')
    .option('--ui', 'open the configuration form, pre-filled, instead of only printing')
    .action(async (options: { ui?: boolean }) => {
      let config = await readStoredConfig<TickTickConfig>('ticktick');
      ...
      if (options.ui) {
        ...
        config = await readStoredConfig<TickTickConfig>('ticktick');
      }
      ...
      for (const line of summarizeConfig(config, { spec: TICKTICK_CONFIG_UI.spec })) console.log(line);

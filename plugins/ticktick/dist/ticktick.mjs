...
async function getV2Token(config, HOST, X_DEVICE) {
  ...
  if (existsSync3(SESSION_CACHE)) {
    try {
      const cached = JSON.parse(readFileSync2(SESSION_CACHE, "utf-8"));
...
function printKV(obj) {
  ...
  for (const [k, v] of entries) {
    const val = v === null ? "-" : Array.isArray(v) ? v.join(", ") : typeof v === "object" ? JSON.stringify(v) : String(v);
...
function printOk(obj) {
  ...
  else if (obj.ids) parts.push(`${obj.ids.length} items`);
  ...
  for (const res of resourceNames) {
    program2.command(res).description(`Manage ${res}`).argument("<action>", "Action to perform (see --help)").argument("[args...]", "Positional arguments for the action").allowUnknownOption(true).allowExcessArguments(true).action(async (action, positionalArgs, _cmdObj) => {
      ...
      function buildXDevice() {
        ...
        const id = config.deviceId || randomBytes2(12).toString("hex");
...
async function authFlow(config, HOST) {
  ...
  const STATE = randomBytes2(8).toString("hex");
  ...
  const tasks = {
    async list(args2, opts2) {
      ...
      if (opts2["group-by-project"]) {
        ...
        for (const [proj, tasks2] of Object.entries(grouped)) {
          console.log(`
${proj} (${tasks2.length})`);
          printTable(tasks2);
        }

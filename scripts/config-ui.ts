#!/usr/bin/env node
/**
 * config-ui.ts — Universal plugin configuration form
 *
 * Launches a local web server with a schema-driven form that writes
 * credentials directly to a config file. Credentials never pass through the LLM.
 *
 * Usage:
 *   node scripts/dist/config-ui.mjs --config <path> --schema '<json>'
 *   node scripts/dist/config-ui.mjs --config <path> --schema-file <path>
 *
 * Schema format:
 *   {
 *     "title": "TickTick",
 *     "description": "Enter your TickTick credentials",
 *     "fields": [
 *       { "key": "HOST", "label": "Host", "type": "select", "options": ["dida365.com", "ticktick.com"], "required": true },
 ...
 * Each field: key (required), label, type (default: text), required, default, options (for select), placeholder, help
 */
...
type FieldType = 'text' | 'password' | 'select' | 'number' | 'textarea' | 'checkbox';
...
const schemaRaw = arg('schema');
const schemaFile = arg('schema-file');
const port = parseInt(arg('port') ?? '0', 10); // 0 = auto-assign

if (!configPath) { console.error('Missing --config <path>'); process.exit(1); }
if (!schemaRaw && !schemaFile) { console.error('Missing --schema or --schema-file'); process.exit(1); }

let schema: Schema;
try {
  const raw = schemaRaw ?? readFileSync(schemaFile as string, 'utf-8');
  schema = JSON.parse(raw) as Schema;
} catch (e) {
  console.error(`Invalid schema: ${(e as Error).message}`);
  process.exit(1);
}
...
const isJson = (configPath as string).endsWith('.json');

function readConfig(path: string): ConfigData {
  if (!existsSync(path)) return {};
  const raw = readFileSync(path, 'utf-8').trim();
  if (!raw) return {};
  if (isJson) {
    try { return JSON.parse(raw) as ConfigData; } catch { return {}; }
  }
  const env: ConfigData = {};
...
function setNested(obj: Record<string, unknown>, dotKey: string, value: string): void {
...
function writeConfig(path: string, data: ConfigData): void {
  mkdirSync(dirname(path), { recursive: true });
  const existing = readConfig(path);
  if (isJson) {
    // Support dot-notation keys for nested JSON
    const existingNested = existing as unknown as Record<string, unknown>;
...
const existing = readConfig(configPath as string);
const langOverride = arg('lang') ?? schema.lang ?? '';
...
function buildHTML(schema: Schema, existing: ConfigData, csrfToken: string): string {
  const title = schema.title ?? '__DEFAULT_TITLE__';
  const description = schema.description ?? '';
  const fields = schema.fields ?? [];

  const fieldRows = fields.map((f: SchemaField) => {
    const key = f.key;
    ...
    const help = f.help ?? '';
    const existingVal = (isJson && key.includes('.') ? getNested(existing as unknown as Record<string, unknown>, key) : existing[key]) ?? '';
    const defaultVal = f.default ?? '';
    const value = (existingVal || defaultVal) as string;

    let inputHTML: string;

    if (type === 'select') {
      const opts = (f.options ?? []).map((o: string) => {
...
const csrfToken: string = randomBytes(16).toString('hex');
...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (req.method === 'POST' && req.url === '/save') {
    ...
    try {
      ...
      const data = JSON.parse(body) as ConfigData;
      writeConfig(configPath as string, data);

      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true }));

      // Output confirmation and exit after response is sent
...
// Timeout: auto-close after 5 minutes of no activity
setTimeout(() => {

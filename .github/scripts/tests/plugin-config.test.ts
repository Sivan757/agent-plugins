...
describe("plugin metadata pipeline", () => {
  ...
  test("packs installable artifacts without source-only files", async () => {
    ...
    expect(existsSync(join(packedRoot, ".codex-plugin"))).toBe(false);
    ...
    // Real hook config is packed via auto-discovery; no root-level placeholder exists.
    expect(existsSync(join(packedRoot, "hooks", "hooks.json"))).toBe(true);
    expect(existsSync(join(packedRoot, "hooks.json"))).toBe(false);
  ...
  test("validation reports drift when generated manifests are edited directly", async () => {
    ...
    const manifestPath = join(root, "plugins", "sample-plugin", ".claude-plugin", "plugin.json");

...
function readConfigFile(cfgPath: string): ConfigData {
...
function writeConfigFile(cfgPath: string, data: ConfigData): void {
  mkdirSync(dirname(cfgPath), { recursive: true });
  const existing = readConfigFile(cfgPath) as unknown as Record<string, unknown>;
...
function buildHTML(
  ...
  const fieldRows = fields.map((f: SchemaField) => {
    ...
    const help = f.help ?? '';
    const existingVal = (key.includes('.') ? getNested(existing as unknown as Record<string, unknown>, key) : existing[key]) ?? '';

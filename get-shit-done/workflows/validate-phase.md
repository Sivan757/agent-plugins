...
```bash
AUDITOR_MODEL=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```

...
```bash
INIT=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```

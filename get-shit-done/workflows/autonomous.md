...
```bash
DETAIL=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```

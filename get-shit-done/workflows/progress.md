...
```bash
INIT=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" init progress)
...
- `STATE=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" progress bar --raw)
```

...
```bash
INIT=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
```bash
UI_PHASE_CFG=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" config-set workflow.nyquist_validation false`

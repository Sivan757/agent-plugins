...
```bash
TRANSITION=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" config-set workflow._auto_chain_active false
```

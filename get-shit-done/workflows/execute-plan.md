...
```bash
INIT=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" phases list --type summaries --raw
...
```bash
NODE_REPAIR=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```
...
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state update-progress

# Record execution metrics
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state record-metric \
...
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state record-session \
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>

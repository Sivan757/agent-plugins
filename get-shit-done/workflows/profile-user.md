...
Run session scan:
```bash
SCAN_RESULT=$(node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Get questions:**
```bash
QUESTIONS=$(node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

...
Use template from `${CLAUDE_PLUGIN_ROOT}/templates/UI-SPEC.md`.
...
Read template: `${CLAUDE_PLUGIN_ROOT}/templates/UI-SPEC.md`
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```

...
```bash
INIT=$(node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" state load)
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```

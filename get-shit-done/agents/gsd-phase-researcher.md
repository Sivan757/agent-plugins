...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
